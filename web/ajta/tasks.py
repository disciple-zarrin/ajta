...
def grab_screenshot(task, domain, yaml_configuration, results_dir, activity_id):
	...
	notification = Notification.objects.all()
	if notification and notification[0].send_scan_status_notif:
		send_notification('ajta is currently gathering screenshots for {}'.format(domain.name))
	...
	if notification and notification[0].send_scan_status_notif:
		send_notification('ajta has finished gathering screenshots for {}'.format(domain.name))
...
def port_scanning(
	...
	if notification and notification[0].send_scan_status_notif:
		...
		send_notification('ajta has finished Port Scanning on {} and has identified {} ports.'.format(domain_name, port_count))
...
def fetch_endpoints(
	...
	'''
		...
		and runs HTTP probe
		ajta has ability to fetch deep urls, meaning url for all the subdomains
	...
	if notification and notification[0].send_scan_status_notif:
		...
		send_notification('ajta has finished gathering endpoints for {} and has discovered *{}* unique endpoints.\n\n{} of those endpoints reported HTTP status 200.'.format(
			domain_name,
			endpoint_count,
...
def perform_osint(scan_history, domain, yaml_configuration, results_dir):
	notification = Notification.objects.all()
	if notification and notification[0].send_scan_status_notif:
		send_notification('ajta has initiated OSINT on target {}'.format(domain.name))
	...
	if notification and notification[0].send_scan_status_notif:
		send_notification('ajta has completed performing OSINT on target {}'.format(domain.name))
